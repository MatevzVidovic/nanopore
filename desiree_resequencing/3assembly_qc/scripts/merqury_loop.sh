...
### eva loop version se zacne TUKAJ
...
###### eva loop version se konca TUKAJ
 ...
 do
     ...
     meryl_db=/scratch/timg/desiree_asm/meryl/desiree_illumina_raw.meryl
     out_base=/scratch/timg/desiree_asm/merqury
     out_dir=$out_base/$out_name
     maxcov=200
     maxcount=15000000
     threads=20
     ...
     # merqury does not recognize path as output dir so you have to cd inside
     cd $out_dir
     conda activate /users/timg/.conda/envs/merqury
     merqury.sh $meryl_db \
         $assembly1 \
         $assembly2 \
         merqury > \
